...
class TelegramNotifier:
    ...
    def send_digest_summary(self, digest: dict, language: str = "en", channel_title: str = "AI News") -> bool:
        ...
        """
        if not self.bot_token or not self.chat_id:
            logger.error("Telegram bot token or chat ID not configured. Skipping.")
            return False
        ...
        for cat in categories:
            ...
            # Show up to 3 stories per category; prefer high-importance ones
        ...
        success = True
        for msg in messages:
            if not self._send_single_message(msg, "HTML"):
                success = False
                break

        if success:
            logger.info("Digest summary sent to Telegram successfully")
        return success
